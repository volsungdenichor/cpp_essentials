...
template <template <class...> class Op, class... Args>
using enable_if_detected = std::enable_if_t<is_detected_v<Op, Args...>>;
...
template <class T>
using HasIstreamOperator = decltype(std::declval<std::istream>() >> std::declval<T>());
...
template <class T>
using Range = std::void_t
    < enable_if_detected<Iterator, decltype(std::begin(std::declval<T>()))>
    , enable_if_detected<Iterator, decltype(std::end(std::declval<T>()))>>;
